...
DECLARE
  ...
  v_action TEXT := 'EVENT_UPDATED';
  v_admin_name TEXT;
BEGIN
  -- Get admin name
  SELECT name INTO v_admin_name FROM users WHERE id = auth.uid();
  
  -- Check if event was started
  IF OLD.status != 'in_progress' AND NEW.status = 'in_progress' THEN
    v_action := 'EVENT_STARTED';
    PERFORM log_audit_action(
      NEW.organization_id,
      auth.uid(),
      v_action,
      ...
      NEW.title,
      jsonb_build_object(
        'admin', COALESCE(v_admin_name, 'Unknown Admin'),
  ...
  -- Track regular updates (title, max_attendees, etc.)
  ...
  END IF;
  IF OLD.max_attendees IS DISTINCT FROM NEW.max_attendees THEN
  ...
  IF OLD.time IS DISTINCT FROM NEW.time THEN
    v_changes := v_changes || jsonb_build_object('time', jsonb_build_object('old', OLD.time, 'new', NEW.time));
  END IF;
  IF OLD.location IS DISTINCT FROM NEW.location THEN
    v_changes := v_changes || jsonb_build_object('location', jsonb_build_object('old', OLD.location, 'new', NEW.location));
  END IF;
  IF OLD.allowed_roles::text IS DISTINCT FROM NEW.allowed_roles::text THEN
    v_changes := v_changes || jsonb_build_object('allowed_roles', jsonb_build_object('old', OLD.allowed_roles, 'new', NEW.allowed_roles));
  END IF;
